...
public class RegisteActivity extends BaseActivity implements View.OnClickListener, MainListPopupWindow.Listener {
	...
	private void isFirstRun() {
		if (!isFirstRun) {
			...
			if (sharedPreferencesUtil.getBoolean(SharedPreferencesUtil.IS_AUTO)) {
				...
				Intent intent = new Intent(RegisteActivity.this, PeopleActivity.class);
	...
	public void onItemClickListener(int position) {
		...
		name.setText(userName);
		/*if (userMap.containsKey(userList.get(position))) {
			userPassword = userMap.get(userList.get(position));
			password.setText(userPassword);
		} else {
			password.setText("");
		}*/
...
//					mainlistpopupwindow.mListPopupWindowAdapter.notifyDataSetChanged();
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		switch (requestCode) {
			case Constant.REQUEST_STORED_CODE :
				if (grantResults.length > 0) {
					if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
						// 创建文件夹
						if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
							File filecc = new File("/sdcard/cc/");
							filecc.mkdirs();
						}
					}
				}
	...
	protected void onResume() {
		super.onResume();
		File headFile = new File("/sdcard/cc/" + userName + ".jpg");
		if (headFile.exists()) {
	...
	TextWatcher textWatcher = new TextWatcher() {
		...
		public void afterTextChanged(Editable s) {
			...
			File headFile = new File("/sdcard/cc/" + s.toString() + ".jpg");
			if (headFile.exists()) {
				CircleHeadPhotoUtil.setCirclePhoto(headFile.getPath(), headImage);
			} else {
				headImage.setImageResource(R.drawable.head);
